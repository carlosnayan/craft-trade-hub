...
interface Props {
  ...
  onChange: (enchantment: number) => void;
}

const ENCHANTMENTS = [0, 1, 2, 3, 4];

export function EnchantmentSelector({ selectedEnchantment, onChange }: Props) {
          ...
          {ENCHANTMENTS.map((ench) => (
            <button
              ...
              className={`w-full text-left px-3 py-2 text-sm hover:bg-secondary transition-colors ${
                selectedEnchantment === ench ? "bg-secondary font-medium" : ""
              }`}
            >
              {ench}
            </button>
