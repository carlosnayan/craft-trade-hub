...
import { pricesFetcher, formatSilver, CITIES, PriceData } from "@/lib/api";
...
export function CraftView({ item }: Props) {
	...
	// 	queryFn: () => pricesFetcher(allItemIds, CITIES),
	...
	const { data, isLoading } = useSWR(
		allItemIds.length ? [allItemIds, server] : null,
		pricesFetcher,
	);
	...
	const cityData = CITIES.map((city) => {
		...
		recipe.forEach((r) => {
			...
			costBuyOrder +=
				r.quantity *
				(rd.buy_price_max || rd.sell_price_min) *
				effectiveMultiplier;
		});
		...
		return {
			...
			revenueBuyOrder,
			profitInstant:
				hasResourceData && revenueSell
					? Math.round(revenueSell - costInstantBuy)
					: null,
			profitOrder:
				hasResourceData && revenueBuyOrder
					? Math.round(revenueBuyOrder - costBuyOrder)
					: null,
			hasResourceData,
		};
	});
	...
	return (
		<div className="space-y-6 animate-fade-in">
			...
			<div className="flex flex-wrap gap-3">
				...
				<Toggle
					label={t("cityBonus")}
					checked={useCityBonus}
					onChange={setUseCityBonus}
				/>
				<Toggle
					label={t("eventBonus")}
					checked={useEventBonus}
					onChange={setUseEventBonus}
				/>
										...
										return (
											...
											>
												...
												<div className="text-muted-foreground">
													{formatSilver(rd?.buy_price_max ?? 0)}
												</div>
											</td>
										);
									})}
